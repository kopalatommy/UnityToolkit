...
namespace ProjectWorlds.Geometry._2d
{
    public static class Distance2
    {
        ...
        {
            return Vector2.Distance(point, Closest2.PointLine(point, line));
        }
        ...
        {
            return Vector2.Distance(point, Closest2.PointLine(point, lineOrigin, lineDirection));
        }
        ...
        {
            return Vector2.Distance(point, Closest2.PointRay(point, ray));
        }
        ...
        {
            return Vector2.Distance(point, Closest2.PointRay(point, rayOrigin, rayDirection));
        }
        ...
        {
            return Vector2.Distance(point, Closest2.PointSegment(point, segment));
        }
        ...
        {
            return Vector2.Distance(point, Closest2.PointSegment(point, segmentA, segmentB));
        }
        ...
        private static float PointSegment(Vector2 point, Vector2 segmentA, Vector2 segmentB, Vector2 segmentDirection, float segmentLength)
        {
            ...
            if (pointProjection < -Geometry2.Epsilon)
            {
                return Vector2.Distance(point, segmentA);
            }
            if (pointProjection > segmentLength + Geometry2.Epsilon)
            {
        ...
        {
            return LineLine(lineA.Start, lineA.Direction, lineB.Start, lineB.Direction);
        }
        ...
        {
            if (Mathf.Abs(VectorExtensions.PerpDot(directionA, directionB)) < Geometry2.Epsilon)
            {
                // Parallel
                Vector2 originBToA = originA - originB;
                if (Mathf.Abs(VectorExtensions.PerpDot(directionA, originBToA)) > Geometry2.Epsilon ||
                    Mathf.Abs(VectorExtensions.PerpDot(directionB, originBToA)) > Geometry2.Epsilon)
                {
                    // Not collinear
        ...
        {
            return LineRay(line.Start, line.Difference, ray.origin, ray.direction);
        }
        ...
        public static float LineRay(Vector2 lineOrigin, Vector2 lineDirection, Vector2 rayOrigin, Vector2 rayDirection)
        {
            ...
            float denominator = VectorExtensions.PerpDot(lineDirection, rayDirection);
            float perpDotA = VectorExtensions.PerpDot(lineDirection, rayOriginToLineOrigin);

            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                // Parallel
                float perpDotB = VectorExtensions.PerpDot(rayDirection, rayOriginToLineOrigin);
                if (Mathf.Abs(perpDotA) > Geometry2.Epsilon || Mathf.Abs(perpDotB) > Geometry2.Epsilon)
                {
                    // Not collinear
            ...
            if (rayDistance < -Geometry2.Epsilon)
            {
                // No intersection
                float rayOriginProjection = Vector2.Dot(lineDirection, rayOriginToLineOrigin);
        ...
        {
            return LineSegment(line.Start, line.Difference, segment.a, segment.b);
        }
        ...
        public static float LineSegment(Vector2 lineOrigin, Vector2 lineDirection, Vector2 segmentA, Vector2 segmentB)
        {
            ...
            float denominator = VectorExtensions.PerpDot(lineDirection, segmentDirection);
            float perpDotA = VectorExtensions.PerpDot(lineDirection, segmentAToOrigin);

            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                // Parallel
                // Normalized direction gives more stable results 
                float perpDotB = VectorExtensions.PerpDot(segmentDirection.normalized, segmentAToOrigin);
                if (Mathf.Abs(perpDotA) > Geometry2.Epsilon || Mathf.Abs(perpDotB) > Geometry2.Epsilon)
                {
                    // Not collinear
            ...
            if (segmentDistance < -Geometry2.Epsilon || segmentDistance > 1 + Geometry2.Epsilon)
            {
                // No intersection
        ...
        {
            return LineCircle(line.Start, line.Direction, circle.center, circle.radius);
        }
        ...
        public static float LineCircle(Vector2 lineOrigin, Vector2 lineDirection, Vector2 circleCenter, float circleRadius)
        {
            ...
            if (sqrDistanceToIntersection < -Geometry2.Epsilon)
            {
                // No intersection
        ...
        {
            Vector2 originBToA = originA - originB;
            float denominator = VectorExtensions.PerpDot(directionA, directionB);
            float perpDotA = VectorExtensions.PerpDot(directionA, originBToA);
            float perpDotB = VectorExtensions.PerpDot(directionB, originBToA);
            ...
            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                ...
                {
                    // Not collinear
                    if (!codirected && originBProjection < Geometry2.Epsilon)
                    {
                ...
                {
                    if (originBProjection < Geometry2.Epsilon)
                    {
                        // No intersection
                        return Vector2.Distance(originA, originB);
                    }
            ...
            if (distanceA < -Geometry2.Epsilon || distanceB < -Geometry2.Epsilon)
            {
                // No intersection
                if (codirected)
                {
                    float originAProjection = Vector2.Dot(directionB, originBToA);
                    if (originAProjection > -Geometry2.Epsilon)
                    {
                ...
                {
                    if (distanceA > -Geometry2.Epsilon)
                    {
                        float originBProjection = -Vector2.Dot(directionA, originBToA);
                        if (originBProjection > -Geometry2.Epsilon)
                        {
                    ...
                    else if (distanceB > -Geometry2.Epsilon)
                    {
                        float originAProjection = Vector2.Dot(directionB, originBToA);
                        if (originAProjection > -Geometry2.Epsilon)
                        {
        ...
        public static float RaySegment(Vector2 rayOrigin, Vector2 rayDirection, Vector2 segmentA, Vector2 segmentB)
        {
            ...
            Vector2 segmentDirection = segmentB - segmentA;
            float denominator = VectorExtensions.PerpDot(rayDirection, segmentDirection);
            float perpDotA = VectorExtensions.PerpDot(rayDirection, segmentAToOrigin);
            // Normalized direction gives more stable results 
            float perpDotB = VectorExtensions.PerpDot(segmentDirection.normalized, segmentAToOrigin);
            ...
            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                ...
                {
                    // Not collinear
                    if (segmentAProjection > -Geometry2.Epsilon)
                    {
                        float distanceSqr = segmentAToOrigin.sqrMagnitude - segmentAProjection * segmentAProjection;
                        // distanceSqr can be negative
                        return distanceSqr <= 0 ? 0 : Mathf.Sqrt(distanceSqr);
                    }
                    if (segmentBProjection > -Geometry2.Epsilon)
                    {
                ...
                // Collinear
                if (segmentAProjection > -Geometry2.Epsilon || segmentBProjection > -Geometry2.Epsilon)
                {
            ...
            float segmentDistance = perpDotA / denominator;
            if (rayDistance < -Geometry2.Epsilon ||
                segmentDistance < -Geometry2.Epsilon || segmentDistance > 1 + Geometry2.Epsilon)
            {
                // No intersection
                ...
                if (!codirected)
                {
                    GeometryUtils.Swap(ref segmentA, ref segmentB);
                ...
                bool segmentAOnRay = segmentAProjection > -Geometry2.Epsilon;
                bool segmentBOnRay = segmentBProjection > -Geometry2.Epsilon;
                ...
                else if (!segmentAOnRay && segmentBOnRay)
                {
                    ...
                    else if (segmentDistance > 1 + Geometry2.Epsilon)
                    {
                        Vector2 rayPoint = rayOrigin + rayDirection * segmentBProjection;
                        Vector2 segmentPoint = segmentB;
                        return Vector2.Distance(rayPoint, segmentPoint);
        ...
        public static float RayCircle(Vector2 rayOrigin, Vector2 rayDirection, Vector2 circleCenter, float circleRadius)
        {
            ...
            if (centerProjection + circleRadius < -Geometry2.Epsilon)
            {
                // No intersection
            ...
            {
                // No intersection
                if (centerProjection < -Geometry2.Epsilon)
                {
            ...
            if (distanceA < -Geometry2.Epsilon)
            {
                if (distanceB < -Geometry2.Epsilon)
                {
                    // No intersection
                    return Mathf.Sqrt(sqrDistanceToOrigin) - circleRadius;
                }
        ...
        public static float SegmentSegment(Vector2 segment1A, Vector2 segment1B, Vector2 segment2A, Vector2 segment2B)
        {
            ...
            bool segment1IsAPoint = segment1Length < Geometry2.Epsilon;
            bool segment2IsAPoint = segment2Length < Geometry2.Epsilon;
            ...
            direction1.Normalize();
            direction2.Normalize();
            float denominator = VectorExtensions.PerpDot(direction1, direction2);
            float perpDot1 = VectorExtensions.PerpDot(direction1, from2ATo1A);
            float perpDot2 = VectorExtensions.PerpDot(direction2, from2ATo1A);

            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                // Parallel
                if (Mathf.Abs(perpDot1) > Geometry2.Epsilon || Mathf.Abs(perpDot2) > Geometry2.Epsilon)
                {
                    // Not collinear
                    float segment2AProjection = -Vector2.Dot(direction1, from2ATo1A);
                    if (segment2AProjection > -Geometry2.Epsilon &&
                        segment2AProjection < segment1Length + Geometry2.Epsilon)
                    {
                    ...
                    if (segment2BProjection > -Geometry2.Epsilon &&
                        segment2BProjection < segment1Length + Geometry2.Epsilon)
                    {
            ...
            if (distance1 < -Geometry2.Epsilon || distance1 > segment1Length + Geometry2.Epsilon ||
                distance2 < -Geometry2.Epsilon || distance2 > segment2Length + Geometry2.Epsilon)
            {
                // No intersection
                bool codirected = Vector2.Dot(direction1, direction2) > 0;
                Vector2 from1ATo2B;
                if (!codirected)
                {
                    GeometryUtils.Swap(ref segment2A, ref segment2B);
                ...
                bool segment2AIsAfter1A = segment2AProjection > -Geometry2.Epsilon;
                bool segment2BIsBefore1B = segment2BProjection < segment1Length + Geometry2.Epsilon;
                bool segment2AOnSegment1 = segment2AIsAfter1A && segment2AProjection < segment1Length + Geometry2.Epsilon;
                bool segment2BOnSegment1 = segment2BProjection > -Geometry2.Epsilon && segment2BIsBefore1B;
                if (segment2AOnSegment1 && segment2BOnSegment1)
                {
                    if (distance2 < -Geometry2.Epsilon)
                    {
                ...
                else if (!segment2AOnSegment1 && !segment2BOnSegment1)
                {
                    ...
                    {
                        segment1Point = distance1 < -Geometry2.Epsilon ? segment1A : segment1B;
                    }
                    else
                    {
                        // Not on segment
                ...
                {
                    if (distance2 > segment2Length + Geometry2.Epsilon)
                    {
                        segment1Point = segment1A + direction1 * segment2BProjection;
                        segment2Point = segment2B;
                    }
        ...
        private static float SegmentSegmentCollinear(Vector2 leftA, Vector2 leftB, Vector2 rightA)
        {
            ...
            if (Mathf.Abs(rightAProjection) < Geometry2.Epsilon)
            {
                // LB == RA
                // LA------LB
                //         RA------RB
        ...
        public static float SegmentCircle(Vector2 segmentA, Vector2 segmentB, Vector2 circleCenter, float circleRadius)
        {
            ...
            if (segmentLength < Geometry2.Epsilon)
            {
            ...
            if (centerProjection + circleRadius < -Geometry2.Epsilon ||
                centerProjection - circleRadius > segmentLength + Geometry2.Epsilon)
            {
                // No intersection
            ...
            if (sqrDistanceToIntersection < -Geometry2.Epsilon)
            {
                ...
                if (centerProjection > segmentLength + Geometry2.Epsilon)
                {
                    return (circleCenter - segmentB).magnitude - circleRadius;
                }
                return Mathf.Sqrt(sqrDistanceToLine) - circleRadius;
            ...
            bool pointAIsAfterSegmentA = distanceA > -Geometry2.Epsilon;
            bool pointBIsBeforeSegmentB = distanceB < segmentLength + Geometry2.Epsilon;
            ...
            bool pointAIsBeforeSegmentB = distanceA < segmentLength + Geometry2.Epsilon;
            if (pointAIsAfterSegmentA && pointAIsBeforeSegmentB)
            {
                // Point A intersection
                return 0;
            }
            bool pointBIsAfterSegmentA = distanceB > -Geometry2.Epsilon;
