...
namespace ProjectWorlds.Geometry._2d
{
    public static class Intersect2
    {
        ...
        {
            return PointLine(point, line.Start, line.Direction);
        }
        ...
        {
            return PointLine(point, line.Start, line.Direction, out side);
        }
        ...
        {
            float perpDot = VectorExtensions.PerpDot(point - lineOrigin, lineDirection);
            return -Geometry2.Epsilon < perpDot && perpDot < Geometry2.Epsilon;
        }
        ...
        public static bool PointRay(Vector2 point, Vector2 rayOrigin, Vector2 rayDirection)
        {
            ...
            float perpDot = VectorExtensions.PerpDot(toPoint, rayDirection);
            return -Geometry2.Epsilon < perpDot && perpDot < Geometry2.Epsilon &&
                   Vector2.Dot(rayDirection, toPoint) > -Geometry2.Epsilon;
        }
        ...
        public static bool PointRay(Vector2 point, Vector2 rayOrigin, Vector2 rayDirection, out int side)
        {
            ...
            side = 0;
            return Vector2.Dot(rayDirection, toPoint) > -Geometry2.Epsilon;
        }
        ...
        public static bool PointSegment(Vector2 point, Vector2 segmentA, Vector2 segmentB)
        {
            ...
            if (sqrSegmentLength < Geometry2.Epsilon)
            {
            ...
            float perpDot = VectorExtensions.PerpDot(toPoint, segmentDirection);
            if (-Geometry2.Epsilon < perpDot && perpDot < Geometry2.Epsilon)
            {
                float pointProjection = Vector2.Dot(segmentDirection, toPoint);
                return pointProjection > -Geometry2.Epsilon &&
                       pointProjection < Mathf.Sqrt(sqrSegmentLength) + Geometry2.Epsilon;
            }
            return false;
        }
        ...
        private static bool PointSegment(Vector2 point, Vector2 segmentA, Vector2 segmentDirection, float sqrSegmentLength)
        {
            ...
            if (-Geometry2.Epsilon < perpDot && perpDot < Geometry2.Epsilon)
            {
                float pointProjection = Vector2.Dot(segmentDirection, toPoint);
                return pointProjection > -Geometry2.Epsilon &&
                       pointProjection < segmentLength + Geometry2.Epsilon;
            }
            return false;
        }
        ...
        {
            if (Mathf.Abs(segmentA.x - segmentB.x) < Geometry2.Epsilon)
            {
                // Vertical
        ...
        public static bool PointCircle(Vector2 point, Vector2 circleCenter, float circleRadius)
        {
            ...
            return (point - circleCenter).magnitude < circleRadius + Geometry2.Epsilon;
        }
        ...
        {
            return LineLine(lineA.Start, lineA.Direction, lineB.Start, lineB.Direction, out IntersectionLineLine2 intersection);
        }
        ...
        {
            return LineLine(lineA.Start, lineA.Direction, lineB.Start, lineB.Direction, out intersection);
        }
        ...
        {
            return LineRay(line.Start, line.Direction, ray.origin, ray.direction, out IntersectionLineRay2 intersection);
        }
        ...
        {
            return LineRay(line.Start, line.Direction, ray.origin, ray.direction, out intersection);
        }
        ...
        {
            return LineSegment(line.Start, line.Direction, segment.a, segment.b, out IntersectionLineSegment2 intersection);
        }
        ...
        {
            return LineSegment(line.Start, line.Direction, segment.a, segment.b, out intersection);
        }
            ...
            out IntersectionLineSegment2 intersection)
        {
            ...
            if (Mathf.Abs(denominator) < Geometry2.Epsilon)
            {
                ...
                // Collinear
                bool segmentIsAPoint = segmentDirection.sqrMagnitude < Geometry2.Epsilon;
                if (segmentIsAPoint)
                {
        ...
        {
            return LineCircle(line.Start, line.Direction, circle.center, circle.radius, out IntersectionLineCircle intersection);
        }
        ...
        {
            return LineCircle(line.Start, line.Direction, circle.center, circle.radius, out intersection);
        }
            ...
            out IntersectionSegmentSegment2 intersection)
        {
            ...
            bool segment1IsAPoint = sqrSegment1Length < Geometry2.Epsilon;
            bool segment2IsAPoint = sqrSegment2Length < Geometry2.Epsilon;
            ...
            if (distance1 < -Geometry2.Epsilon || distance1 > 1 + Geometry2.Epsilon)
            {
                intersection = IntersectionSegmentSegment2.None();
                return false;
            }
            ...
            if (distance2 < -Geometry2.Epsilon || distance2 > 1 + Geometry2.Epsilon)
            {
                intersection = IntersectionSegmentSegment2.None();
                return false;
            }
            ...
            out IntersectionSegmentCircle intersection)
        {
            ...
            if (segmentLength < Geometry2.Epsilon)
            {
                ...
                if (distanceToPoint < circleRadius + Geometry2.Epsilon)
                {
                    if (distanceToPoint > circleRadius - Geometry2.Epsilon)
                    {
            ...
            {
                if (centerProjection < -Geometry2.Epsilon ||
                    centerProjection > segmentLength + Geometry2.Epsilon)
                {
                    intersection = IntersectionSegmentCircle.None();
                    return false;
                }
            ...
            out IntersectionCircleCircle intersection)
        {
            ...
            if (distanceFromBtoASqr < Geometry2.Epsilon)
            {
                if (Mathf.Abs(radiusA - radiusB) < Geometry2.Epsilon)
                {
            ...
            if (Mathf.Abs(distanceFromBtoA - sumOfRadii) < Geometry2.Epsilon)
            {
            ...
            if (Mathf.Abs(distanceFromBtoA - differenceOfRadiiAbs) < Geometry2.Epsilon)
            {
