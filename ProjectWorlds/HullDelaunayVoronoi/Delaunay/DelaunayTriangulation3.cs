...
namespace ProjectWorlds.HullDelaunayVeronoi.Delaunay
{
    ...
    public class DelaunayTriangulation3<VERTEX> : DelaunayTriangulation<VERTEX> where VERTEX : class, IVertex, new()
    {
        private float[,] matrixBuffer;

        public DelaunayTriangulation3() : base(3)
        {
            matrixBuffer = new float[4, 4];
        }
        ...
        {
            return matrixBuffer[r0, c0] * (matrixBuffer[r1, c1] * matrixBuffer[r2, c2] - matrixBuffer[r2, c1] * matrixBuffer[r1, c2]) -
                    matrixBuffer[r0, c1] * (matrixBuffer[r1, c0] * matrixBuffer[r2, c2] - matrixBuffer[r2, c0] * matrixBuffer[r1, c2]) +
                    matrixBuffer[r0, c2] * (matrixBuffer[r1, c0] * matrixBuffer[r2, c1] - matrixBuffer[r2, c0] * matrixBuffer[r1, c1]);
        }

        private float Determinant()
        {
            return (matrixBuffer[0, 0] * MINOR(1, 2, 3, 1, 2, 3) -
                    matrixBuffer[0, 1] * MINOR(1, 2, 3, 0, 2, 3) +
                    matrixBuffer[0, 2] * MINOR(1, 2, 3, 0, 1, 3) -
                    matrixBuffer[0, 3] * MINOR(1, 2, 3, 0, 1, 2));
        }
        ...
        private DelaunayCell<VERTEX> CreateCell(Simplex<VERTEX> simplex)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                matrixBuffer[i, 2] = verts[i].Position[2];
                matrixBuffer[i, 3] = 1;
            }
            float a = Determinant();
            ...
            for (int i = 0; i < 4; i++)
            {
                matrixBuffer[i, 3] = verts[i].Position[2];
            }
            float c = Determinant();
