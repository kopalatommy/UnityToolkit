...
namespace ProjectWorlds.HullDelaunayVeronoi.Hull
{
    ...
    public class ConvexHull<VERTEX> : IConvexHull<VERTEX> where VERTEX : class, IVertex, new()
    {
        ...
        public bool Contains(VERTEX vertex)
        {
            ...
            {
                if (HullMathHelper<VERTEX>.GetVertexDistance(vertex, Simplexs[i]) >= PLANE_DISTANCE_TOLERANCE)
                {
                    return false;
                }
            }
        ...
        private List<VERTEX> FindInitialPoints(List<VERTEX> extremes)
        {
            ...
            for (int i = 0; i < extremes.Count - 1; i++)
            {
                ...
                for (int j = i + 1; j < extremes.Count; j++)
                {
                    ...
                    HullMathHelper<VERTEX>.SubtractFast(a.Position, b.Position, temp);

                    float dist = HullMathHelper<VERTEX>.LengthSquared(temp);
        ...
        private bool CalculateFacePlane(SimplexWrap<VERTEX> face)
        {
            ...
            HullMathHelper<VERTEX>.FindNormalVector(vertices, normal);
        ...
        private void TraverseAffectedFaces(SimplexWrap<VERTEX> currentFace)
        {
            ...
            while (Buffer.TraverseStack.Count > 0)
            {
                ...
                for (int i = 0; i < Dimension; i++)
                {
                    ...
                    if (adjFace.Tag == 0 && HullMathHelper<VERTEX>.GetVertexDistance(Buffer.CurrentVertex, adjFace) >= PLANE_DISTANCE_TOLERANCE)
                    {
        ...
        {
            float distance = HullMathHelper<VERTEX>.GetVertexDistance(v, face);
