...
namespace ProjectWorlds.HullDelaunayVeronoi.Hull
{
    ...
    /// </summary>
    public class ObjectBuffer<VERTEX> where VERTEX : class, IVertex, new()
    {
        ...
        public void AddInput(IList<VERTEX> input, bool assignIds, bool checkInput)
        {
            ...
            if (checkInput)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (input[i].Dimensions != Dimension)
                    {
                        throw new ArgumentException("Input vertex is not the correct dimension", input[i].Dimensions.ToString());
                    }
